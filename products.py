...
class Commodities():
    ...
    def __init__(self, window):
        ...
        self.back_img = Image.open('images/commodities_in1.png')
        ...
        self.heading = Label(self.left, text="Product Information", font=('arial 30 bold'), fg='HotPink4', bg="white")
        ...
        # Search Button
        self.search = Button(self.left, text="Click to search", width=15, height=2, font=('arial 10 bold'), bg="light gray", command=self.search_product)
    ...
    def add_product(self):
        """This function is used to add a new product into the commodities table of our Grocery Store Management system"""
        ...
        if self.val1 == '' or self.val2 == '' or self.val3 == '' or self.val4 == '':
            ...
            query_comm = "INSERT INTO Products(productID, productName, stock, price) VALUES(%s, %s, %s, %s)"
    ...
    def searchNow(self):
        ...
        if selected == "Product_No":
            ...
            sql_query = "SELECT * FROM Products WHERE productName=%s"
        elif selected == "Prod_Quantity":
            sql_query = "SELECT * FROM Products WHERE stock=%s"
        elif selected == "Prod_Price":
            sql_query = "SELECT * FROM Products WHERE price=%s"
    ...
    def editNow(self):
        """This function is called when we click on the change button. We need to select the product and then click on the edit button.
        This function then provides us the option to update or delete"""
        try:
            if self.item_text:
                self.cProduct_No = Label(self.search_product, text="Product_No", font=('arial 10 bold'), fg='black')
                ...
                self.cProd_Quantity = Label(self.search_product, text="Prod_Quantity", font=('arial 10 bold'), fg='black')
                ...
                self.cProd_Price = Label(self.search_product, text="Prod_Price", font=('arial 10 bold'), fg='black')
    ...
    def updateNow(self):
        ...
        update_query = "UPDATE Products SET productName = %s, stock = %s, price = %s WHERE productID = %s"
    ...
    def deleteNow(self):
        ...
        try:
            if my_var:  # True if the yes button is clicked
                mycursor.execute("DELETE FROM Products WHERE productID= " + Pro_NO)
                conn.commit()
                self.clear()
                tkinter.messagebox.showinfo("Success", "Successfully Deleted", parent=self.search_product)
        except:
            tkinter.messagebox.showinfo("Warning", "Products exist in this Store", parent=self.search_product)
