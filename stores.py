...
class Location():
    ...
    def __init__(self, window):
        ...
        self.back_img = Image.open('images/locations_in1.png')
        ...
        self.background_label.place(x=250, y=-300, relwidth=1, relheight=1)
    ...
    def add_location(self):
        """This function is used to add a new location into the Stores table of our Grocery Store Management system"""
        ...
        if self.val1 == '' or self.val2 == '' or self.val3 == '' or self.val4 == '':
            ...
            query_loc = "INSERT INTO Stores(storeID, address, name, phoneNumber) VALUES(%s, %s, %s, %s)"
    ...
    def searchNow(self):
        """This function helps us to search for any data in Stores table"""
        selected = self.drop.get()
        if selected == "Location ID":
            sql_query = "SELECT * FROM Stores WHERE storeID=%s"
        elif selected == "Zipcode":
            sql_query = "SELECT * FROM Stores WHERE address=%s"
        elif selected == "City":
            sql_query = "SELECT * FROM Stores WHERE name=%s"
        elif selected == "Street":
            sql_query = "SELECT * FROM Stores WHERE phoneNumber=%s"
    ...
    def editNow(self):
        """This function is called when we click on the change button. We need to select the location and then click on the edit button.
        ...
        try:
            if self.item_text:
                ...
                self.lcity = Label(self.search_location, text="City", font=('arial 10 bold'), fg='black')
                ...
                self.lstreet = Label(self.search_location, text="Street", font=('arial 10 bold'), fg='black')
    ...
    def updateNow(self):
        """Used to update the Stores table"""
        update_query = "UPDATE Stores SET address = %s, name = %s, phoneNumber = %s WHERE storeID = %s"
    ...
    def deleteNow(self):
        """Used to delete data from Stores table only if there are no employees working in that location"""
        ...
        try:
            if my_var:  # True if yes button is clicked
                mycursor.execute("DELETE FROM Stores WHERE storeID= " + "\"" + loc_id + "\"")
                conn.commit()
                self.clear()
